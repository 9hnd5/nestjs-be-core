...
import getInstance, { RedisCachingService as CachingService } from "modules/caching/redis.caching.service";
...
export class BaseController {
  ...
  public async getUserSession(accessToken: string): Promise<Session | null> {
    ...
    var key = tenantCode
      ? `${tenantCode.toLowerCase()}:${accessToken}`
      : accessToken;
    key = `A:${key}`;
  ...
  public async getSession(refreshToken: string): Promise<Session | null> {
    var tenantCode = this.scopeVariable.tenantCode;
    var key = tenantCode
      ? `${tenantCode.toLowerCase()}:${refreshToken}`
      : refreshToken;
    key = `R:${key}`;
